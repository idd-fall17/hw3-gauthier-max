...
def sling_action(distance_band, angle_band):
    ...
    global x_mouse
    global y_mouse
    x_mouse = sling_x - math.cos(angle_band) * distance_band
    y_mouse = sling_y - math.sin(angle_band) * distance_band
    ...
    # Code added by me
    #ser = serial.Serial('/dev/cu.usbserial-A106TGIN')
    #print "Serial Data"
    #print ser.readline()
    #print "===="
    
    #print mouse_distance
    mouse_distance = distance_band
    #mouse_distance = 73
    ...
    if dx == 0:
        dx = 0.00000000000001
    angle = math.atan((float(dy))/dx) # Potentiometer reading value
    angle = angle_band
    #print angle
    #angle = 0
    #angle = -1.0
...
def read_from_port():
    return ['0.9', '0.2']
    while True:
        ser = serial.Serial('/dev/cu.usbserial-A106TGIN', baudrate=9600, timeout=0.05)
        dataReturn = ser.readline().split()
        if len(dataReturn) != 0:
            return dataReturn
...
while running:
    ...
    dataArray = dataSerial.split()
    
    #dataArray = read_from_port()
    #print dataArray
    if len(dataArray) == 2:
        distance_band = dataArray[0]
        angle_band = dataArray[1]
        #print angle_band
    ...
    if distance_band != "":
        ...
        if (distance_band_float < 0.6 or distance_band_float > 1.15 or angle_band_float < 1.8 or angle_band_float > 2 ):
            print "ERROR"
            print distance_band_float
            print angle_band_float
            #error = 1
        else:
            #error = 0
            translated_distance = translate(distance_band_float, 0.6, 1.15, 0, 90)
            translated_angle = translate(angle_band_float, 1.8, 2, -math.pi / 2, math.pi / 2)
